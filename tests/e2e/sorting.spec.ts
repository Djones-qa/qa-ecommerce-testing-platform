...
test.describe('Product Sorting', () => {
  ...
  // Log in once, store auth state, reuse across tests
  test.beforeEach(async ({ page }) => {
    const loginPage = new LoginPage(page);
    await loginPage.goto();
    await loginPage.login(USERS.standard.username, USERS.standard.password);

    // Wait for the full React render — inventory items prove the SPA has mounted
    await page.waitForSelector('.inventory_item', { timeout: 60000 });
  });

  test('sort A to Z is default order', async ({ page }) => {
